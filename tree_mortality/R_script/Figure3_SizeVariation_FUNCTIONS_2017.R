  ...
  ################################################
{
  ...
  inc=allmort$status.2015=="Live" #Include only live stems
  mort=ifelse(allmort$status.2016=="Dead",1,0) #Mark trees as alive (0) or dead (1)
  timeint=(allmort$date.2016-allmort$date.2015)/365.25 #Calculate time interval
  #For this census, dbh values were measured for dead trees
  #Select the highest value between agb.2013 and agb.2016 (in case of bark loss, etc.)
  agb.best<-(pmax(allmort$agb.2013,allmort$agb.2016,na.rm=T))/timeint
  mind=data.frame(gx=allmort$gx[inc],gy=allmort$gy[inc],dinit=allmort$dbh.2013[inc],mort=mort[inc],timeint=timeint[inc],agb=agb.best[inc],tag=allmort$tag[inc],stemID=allmort$stemID[inc])
  return(mind)
} # end indcalcmort45
  ...
  ################################################
{
  ...
  inc=allmort$status.2016=="Live" #Include only live stems
  mort=ifelse(allmort$status.2017=="Dead",1,0) #Mark trees as alive (0) or dead (1)
  timeint=(allmort$date.2017-allmort$date.2016)/365.25 #Calculate time interval
  #For this census, dbh values were measured for dead trees
  #Select the highest value between agb.2013 and agb.2016 (in case of bark loss, etc.)
  agb.best<-(pmax(allmort$agb.2013,allmort$agb.2017,na.rm=T))/timeint
  mind=data.frame(gx=allmort$gx[inc],gy=allmort$gy[inc],dinit=allmort$dbh.2013[inc],mort=mort[inc],timeint=timeint[inc],agb=agb.best[inc],tag=allmort$tag[inc],stemID=allmort$stemID[inc])
  return(mind)
} # end indcalcmort56
...
graphmort=function(outfilestem="Rate") 
{
  ...
  pchs=c(16,4,17,15,18) #Shapes
  ltys=c(1,1,1,1,1) #Line type
  space=c(.5,2,3.5,5,6.5) #Spaces between lines in the legend
  ...
  ncensus<-c("2008-2013","2013-2014","2014-2015", "2015-2016", "2016-2017")
  ...
  windowsFonts(
    A=windowsFont("Arial"))
  tiff(file = paste0(Output_data_location_Graphs, "Figure3_2017.tiff"), width = 7.5, height = 8.75, units = "in", res = 600, family ="A", pointsize = 12,compression="lzw")
  ...
  for (i in 1:5) {
    ...
    if (i==1) { #2008-2013
      ...
      sum2=summary(logmod3) ###lines altered
      p2.value<-pf(sum2$fstatistic[1], sum2$fstatistic[2], sum2$fstatistic[3],sum2$fstatistic[4],sum2$fstatistic[5],lower.tail = FALSE)
    ...
    if (i==2) #2013-2014
    {
      ...
      lines(exp(logypred)~x, col=cols[k]) 
      sum=summary(logmod)
      p.value<-pf(sum2$fstatistic[1], sum2$fstatistic[2], sum2$fstatistic[3],sum2$fstatistic[4],sum2$fstatistic[5],lower.tail = FALSE)
      cis<-confint(logmod,'log(x)',level=0.95,method="boot",nsim=1000)
      cisy<-confint(logmod,'(Intercept)',level=0.95,method="boot",nsim=1000)
      #cis3<-confint(logmod,'x',level=0.95,method="boot",nsim=1000)
      stats=data.frame(census=ncensus[i],cutoff="100",r2=sum$r.squared,r2adj=sum$adj.r.squared,p.value=p.value,b=sum$coefficients[[2]],bCIlo=cis[1],bCIhi=cis[2],b_p=sum$coefficients[2,4],y0=sum$coefficients[[1]],yCIlo=cisy[1],yCIhi=cisy[2],y0_p=sum$coefficients[1,4])
  ...
  write.csv(Figure1graphstats, paste0(Output_data_location_Stats, "Figure3graphstatsPowFit_2017.csv"))
...
graphmortU=function(outfilestem="Rate") 
{
  ...
  Figure1graphstats<-merge(graphstats,agbgraphstats)
  write.table(Figure1graphstats,file= paste0(Output_data_location_Stats,"Figure1graphstats.txt"))
  mtext(expression("Diameter (cm)"), side = 1, outer = TRUE, line = 3)#, cex = 1.5,)
